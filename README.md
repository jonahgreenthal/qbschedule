...
## Use
The files are intended to be pretty self-documenting. If you're already fairly comfortable with LaTeX, just download `qbschedule.cls` and the template `.tex` file(s) that you want, open them up in an editor, and do what it looks like you should do. Make sure to compile with `xetex`. More detailed instructions follow.
...
- Save the file.
- Compile the file by running `xetex` on it. That is, open a terminal or command prompt, navigate to the folder you are working in, and run the command `xetex theFileName.tex`.
