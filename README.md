...
## Use
The files are intended to be pretty self-documenting. If you're already fairly comfortable with LaTeX, just download `qbschedule.cls` and the template `.tex` file(s) that you want, open them up in an editor, and do what it looks like you should do. More detailed instructions follow.
...
- Save the file.
- Compile the file by running `pdflatex` on it. That is, open a terminal or command prompt, navigate to the folder you are working in, and run the command `pdflatex theFileName.tex`.
...
- [fixltx2e](http://www.ctan.org/pkg/fixltx2e) (core package) to correct some bugs in LaTeX
- [fontenc](http://www.ctan.org/pkg/fontenc) (core package) so that Latin Modern works
- [inputenc](http://www.ctan.org/pkg/inputenc) (core package) so that the input can be UTF-8
...
- [cmap](http://www.ctan.org/pkg/cmap) for making the output PDFs searchable
